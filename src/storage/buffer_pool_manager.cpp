...
void BufferPoolManager::update_page(Page *page, PageId new_page_id, frame_id_t new_frame_id) {
    ...
    if (page->is_dirty_) {
        ...
        disk_scheduler_->write_page_schedule(page->get_page_id().fd, page->get_page_id().page_no, page->get_data(), PAGE_SIZE);
    ...
    memset(page->data_, 0, PAGE_SIZE);  // Reset page data
    page->pin_count_ = 0;
    page->is_dirty_ = false;
    lck.unlock();
}
...
Page* BufferPoolManager::fetch_page(PageId page_id) {
    ...
    // std::scoped_lock<std::mutex> guard_(latch_);
    std::unique_lock<std::mutex> lck(latch_);
    ...
    disk_scheduler_->read_page(page->get_page_id().fd,page->get_page_id().page_no,page->get_data(),PAGE_SIZE);
    page->pin_count_++;
    replacer_->pin(frame_id);
    return page;
}
...
bool BufferPoolManager::flush_page(PageId page_id) {
    ...
    disk_scheduler_->write_page_schedule(page_id.fd, page_id.page_no,page->data_,PAGE_SIZE);
    page->is_dirty_ = false;
    return true;
}
...
bool BufferPoolManager::delete_page(PageId page_id) {
    ...
    if (page->is_dirty_) {
        // disk_manager_->write_page(page->get_page_id().fd, page->get_page_id().page_no, page->data_, PAGE_SIZE);
        disk_scheduler_->write_page_schedule(page->get_page_id().fd, page->get_page_id().page_no, page->data_, PAGE_SIZE);
    }
...
void BufferPoolManager::flush_all_pages(int fd) {
    ...
    for (auto& entry : page_table_) {
        ...
        if (fd == page_id.fd) {
            ...
            disk_scheduler_->write_page_schedule(fd, page_id.page_no, page->data_, PAGE_SIZE);
            page->is_dirty_ = false;
        }
    }
}