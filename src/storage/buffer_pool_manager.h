...
class BufferPoolManager {
   ...
   public:
    ...
    BufferPoolManager(size_t pool_size, DiskManager *disk_manager, LogManager *log_manager, DiskScheduler *disk_scheduler)
            : pool_size_(pool_size), disk_manager_(disk_manager),log_manager_(log_manager),disk_scheduler_(disk_scheduler) {
        // 为buffer pool分配一块连续的内存空间
        pages_ = new Page[pool_size_];
        // 可以被Replacer改变
        if (REPLACER_TYPE.compare("LRU"))
