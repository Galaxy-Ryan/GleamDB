 ...
 */
 // we need finish all async tasks before close_file
void DiskManager::close_file(int fd) {
    ...
    if(fd_map_.find(fd) != fd_map_.end()) {
        std::unique_lock<std::mutex> sig(fd_latch_);
        cv_.wait(sig,[&](){return fd_map_[fd] == 0;});
    }

    if (close(fd) == -1) {
        throw InternalError("DiskManager::close_file Error: close failed");
    }
