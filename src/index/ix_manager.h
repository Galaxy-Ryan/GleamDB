...
class IxManager {
   ...
   public:
    IxManager(DiskManager *disk_manager, BufferPoolManager *buffer_pool_manager,DiskScheduler *disk_scheduler = nullptr)
        : disk_manager_(disk_manager), buffer_pool_manager_(buffer_pool_manager),disk_scheduler_(disk_scheduler) {}
    ...
    void create_index(const std::string &filename, const std::vector<ColMeta>& index_cols) {
        ...
        disk_scheduler_->write_page_schedule(fd, IX_FILE_HDR_PAGE, data, fhdr->tot_len_);
        ...
        // Create leaf list header page and write to file
        {
            ...
            *phdr = {
            ...
            disk_scheduler_->write_page_schedule(fd, IX_LEAF_HEADER_PAGE, page_buf, PAGE_SIZE);
        }
        ...
        // Create root node and write to file
        {
            ...
            *phdr = {
            ...
            disk_scheduler_->write_page_schedule(fd, IX_INIT_ROOT_PAGE, page_buf, PAGE_SIZE);
        }
    ...
    std::unique_ptr<IxIndexHandle> open_index(const std::string &filename, const std::vector<ColMeta>& index_cols) {
        std::string ix_name = get_index_name(filename, index_cols);
        int fd = disk_manager_->open_file(ix_name);
        return std::make_unique<IxIndexHandle>(disk_manager_, buffer_pool_manager_, fd,disk_scheduler_);
    }
    ...
    void close_index(const IxIndexHandle *ih) {
        ...
        disk_scheduler_->write_page_schedule(ih->fd_, IX_FILE_HDR_PAGE, data, ih->file_hdr_->tot_len_);
