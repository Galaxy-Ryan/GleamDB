...
class RmManager {
   ...
   public:
    RmManager(DiskManager *disk_manager, BufferPoolManager *buffer_pool_manager,DiskScheduler *disk_scheduler = nullptr)
    ...
    void create_file(const std::string& filename, int record_size) {
        ...
        disk_scheduler_->write_page_schedule(fd, RM_FILE_HDR_PAGE, (char *)&file_hdr, sizeof(file_hdr));
    ...
    std::unique_ptr<RmFileHandle> open_file(const std::string& filename) {
        int fd = disk_manager_->open_file(filename);
        return std::make_unique<RmFileHandle>(disk_manager_, buffer_pool_manager_, fd,disk_scheduler_);
    }
    /**
...
//                                  sizeof(file_handle->file_hdr_));
        disk_scheduler_->write_page_schedule(file_handle->fd_, RM_FILE_HDR_PAGE, (char *)&file_handle->file_hdr_,sizeof(file_handle->file_hdr_));
